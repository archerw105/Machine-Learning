...
class Network: 
    ...
    def backprop(self, x, y):
        ...
        #backward pass
        delta = self.cost_derivative(y, activations[-1]) * sigmoid_prime(zs[-1])#Hadamard product
        nabla_w[-1] = np.dot(delta, activations[-2].transpose()) #creates matrix
        nabla_b[-1] = delta
        for l in range(2, self.num_layers):
            z = zs[-l]
            delta = np.dot(self.weights[-l+1].transpose(), delta) * sigmoid_prime(z) #double-check
